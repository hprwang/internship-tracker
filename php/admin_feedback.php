...
$feedbackList = $db->query("
    SELECT f.*, s.full_name as student_name, sup.full_name as supervisor_name
    FROM supervisor_feedback f
    JOIN users s ON f.student_id = s.id
    JOIN users sup ON f.supervisor_id = sup.id
...
<head>
  ...
  <title>InternTrack — Feedback</title>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .feedback-header { display: flex; justify-content: space-between; margin-bottom: 1rem; }
    .feedback-names { display: flex; gap: 1rem; }
    .feedback-from { font-size: 0.85rem; color: var(--text-muted); }
    .feedback-from span { color: var(--green-neon); }
    .feedback-rating { color: #FACC15; }
    .feedback-content { color: var(--text-secondary); line-height: 1.7; }
    .feedback-date { font-size: 0.75rem; color: var(--text-muted); margin-top: 1rem; }
  ...
  <div class="admin-layout">
    <aside class="sidebar">
      ...
      <div class="nav-section">
        ...
        <nav class="nav-menu">
          ...
          <a href="admin_feedback.php" class="nav-item active"><span class="icon">💬</span> Feedback</a>
    ...
    <main class="main-content">
      <div class="page-header">
        <div>
          <h1 class="page-title">Supervisor <span>Feedback</span></h1>
          <p class="page-subtitle">View feedback from supervisors to students</p>
        </div>
      </div>
      ...
      <div class="feedback-list">
        ...
        <div class="feedback-card">
          <div class="feedback-header">
            <div class="feedback-names">
              <div class="feedback-from">From: <span><?= e($fb['supervisor_name']) ?></span> To: <span><?= e($fb['student_name']) ?></span></div>
            </div>
            <div class="feedback-rating">
              <?php for ($i = 1; $i <= 5; $i++): ?>
              <span<?= $i <= $fb['rating'] ? ' style="color:#FACC15"' : '' ?>>★</span>
              <?php endfor; ?>
            </div>
          </div>
