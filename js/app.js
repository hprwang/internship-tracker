...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ── Neon Parallax Starfield (Cyberpunk) ─────────────────────────
  // Runs on any page that includes: <canvas id="starfield"></canvas>
  const canvas = document.getElementById('starfield');
  if (canvas) {
    try { initNeonStarfield(canvas); } catch (e) { /* silent */ }
  }
});
 ...
 * - green stars (twinkle)
 * - floating particles
 * - cosmic dust + subtle light trails
 * High-performance: capped entities, uses RAF, recycles positions for seamless looping.
 */
function initNeonStarfield(canvas) {
  const ctx = canvas.getContext('2d', { alpha: true });
  if (!ctx) return;

  const reduceMotion = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;

  // Handle sizing with DPR
  const state = {
  ...
  const COLORS = {
    ...
    neon: 'rgba(74,222,128,1)',
    lime: 'rgba(167,243,208,1)',
    glow: 'rgba(34,197,94,.35)',
    dust: 'rgba(16,185,129,.12)',
    haze: 'rgba(34,197,94,.06)'
  };
  ...
  // Re-compute counts on resize (keeps perf stable)
  function rebuild() {
    const area = state.w * state.h;

    const farCount = Math.max(90, Math.floor(area / 16000));   // deep background stars
    const midCount = Math.max(70, Math.floor(area / 12000));   // twinkling
    const nearCount = Math.max(50, Math.floor(area / 9000));   // larger + more trails
    const particleCount = Math.max(45, Math.floor(area / 22000)); // floating particles
    const dustCount = Math.max(120, Math.floor(area / 26000));    // cosmic dust

    layers.farStars.length = 0;
    layers.midStars.length = 0;
    layers.nearStars.length = 0;
    layers.particles.length = 0;
    layers.dust.length = 0;

    // Far stars
    for (let i = 0; i < farCount; i++) {
      layers.farStars.push({
    ...
    // Near stars (glow + light trails)
    for (let i = 0; i < nearCount; i++) {
      layers.nearStars.push({
        x: Math.random() * state.w,
        y: Math.random() * state.h,
        vx: rand(-0.12, 0.12),
        vy: rand(0.15, 0.45),
  ...
  function resize() {
    ...
    state.w = Math.max(320, Math.floor(rect.width || window.innerWidth));
    state.h = Math.max(480, Math.floor(rect.height || window.innerHeight));
  ...
  function onPointerMove(e) {
    const x = (e.clientX || state.w * 0.5) / state.w;
    const y = (e.clientY || state.h * 0.35) / state.h;
    state.pointerX = Math.min(1, Math.max(0, x));
    state.pointerY = Math.min(1, Math.max(0, y));
  }

  // Seamless wrap helper (top/bottom/left/right)
  function wrap(ent) {
    // keep within bounds with padding
    const pad = 20;
    if (ent.x < -pad) ent.x = state.w + pad;
    if (ent.x > state.w + pad) ent.x = -pad;
    if (ent.y < -pad) ent.y = state.h + pad;
    if (ent.y > state.h + pad) ent.y = -pad;
  }

  // Draw helpers
  function drawStar(ent, twAmp = 1) {
    // Twinkle (phase + time)
    const tw = Math.sin(ent.tw) * 0.5 + 0.5; // 0..1
    const alpha = ent.a * (0.65 + tw * 0.65) * twAmp;
